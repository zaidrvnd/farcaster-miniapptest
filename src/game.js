...
import { Character, Zombie } from './entities.js';
...
const player = new Character(canvas.width / 2, canvas.height / 2);
...
function resetGame() {
  Object.assign(player, new Character(canvas.width / 2, canvas.height / 2));
  bullets = [];
  zombies = [];
  items = [];
  level = 1;
...
function spawnZombie(type) {
  zombies.push(new Zombie(level, type, canvas.width, canvas.height));
}
