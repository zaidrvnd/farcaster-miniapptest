...
function applyItem(type) {
  switch (type) {
    case 'weapon':
      player.damage += 1;
      showMessage('Senjata +1 DMG');
      break;
    case 'armor':
      player.maxHp += 2;
      player.hp += 2;
      showMessage('Armor +2 HP');
      break;
    case 'helmet':
      player.defense += 1;
      showMessage('Helmet +1 DEF');
      break;
    case 'accessory':
      player.speed += 0.2;
      showMessage('Aksesoris +Kecepatan');
      break;
  }
}
...
function grantCard() {
  ...
  card.apply();
  showMessage(`Mendapat ${card.name}!`);
}
